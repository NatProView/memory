...
def draw_backgrounds():
    ...
    restart_button = pygame.draw.rect(screen, Color.accent.value, [415, HEIGHT - 80, 145, 60], 0)
    restart_button_frame = pygame.draw.rect(screen, Color.primary_text.value, [415, HEIGHT - 80, 145, 60], 3, 4)
    ...
    player_turn_text = small_font.render(f"Player {player_turn}'s Turn", True, Color.white.value)
    player1_score_text = small_font.render(f"P1: {player_scores[1]}", True, Color.white.value)
    player2_score_text = small_font.render(f"P2: {player_scores[2]}", True, Color.white.value)
    # screen.blit(turn_text, (20, HEIGHT - 80))
    # screen.blit(matches_text, (200, HEIGHT - 80))
    screen.blit(restart_text, (420, HEIGHT - 73))
    screen.blit(player_turn_text, (20, HEIGHT - 95))
    screen.blit(player1_score_text, (20, HEIGHT - 50))
    screen.blit(player2_score_text, (190, HEIGHT - 50))
...
def is_match(first, second):
    ...
    if spaces[first] == spaces[second]:
        ...
        if correct[row_first][col_first] != 0 and correct[row_second][col_second] != 0:
            correct[row_first][col_first] = MATCHED
            correct[row_second][col_second] = MATCHED
            matched_pairs += 1
            return True
        if correct[row_first][col_first] == CHECKED and correct[row_second][col_second] == CHECKED:
            correct[row_first][col_first] = EMPTY
            correct[row_second][col_second] = EMPTY
    return False
...
while running:
    ...
    if first_guess and second_guess:
        log_debug(f"Player Turn: {player_turn}")
        log_debug(f"Player Score: {player_scores}")
        if is_match(first_guess_num, second_guess_num):
            ...
            update_checked_card(first_guess_num, EMPTY)
            update_checked_card(second_guess_num, EMPTY)
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            for i in range(len(grid)):
                ...
                if button.collidepoint(event.pos) and not first_guess:
                    ...
                    first_guess_num = i
                    log_debug(f"FIRST {i}")
                    update_checked_card(i, CHECKED)
                elif button.collidepoint(event.pos) and not second_guess and first_guess and first_guess_num != i:
                    second_guess = True
                    second_guess_num = i
    ...
    if first_guess:
        update_checked_card(first_guess_num, CHECKED)
